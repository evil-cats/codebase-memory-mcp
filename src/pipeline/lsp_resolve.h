...
/* Разные написания одной семантической цели считаются равными, только если
 * точный поиск в графе приводит к одному материализованному узлу. JVM-проходы
 * дополнительно согласуют файловые QN и QN пакета ниже; одного совпадения хвоста
 * Class.method недостаточно, поскольку одинаковый метод бывает в двух пакетах. */
static inline bool cbm_pipeline_invocation_targets_equal(const char *left_qn, const char *right_qn,
    ...
    /* Две строки с QN пакета, найденные только по хвосту, остаются разными
     * семантическими утверждениями. Хотя бы одна должна точно назвать узел. */
    if (!left_exact && !right_exact) {
        return false;
    }
...
static inline bool cbm_pipeline_invocation_leaf_matches(const CBMResolvedCall *resolved,
    ...
    if (call_leaf && cbm_lsp_callable_leaf_matches(resolved->callee_qn, call_leaf)) {
        return true;
    }
    ...
    return cbm_pipeline_invocation_reason_join_strategy(resolved->strategy) && resolved->reason &&
           call_leaf && cbm_lsp_callable_leaf_matches(resolved->reason, call_leaf);
}
...
/* Внешние промежуточные записи Kotlin должны разрешаться только в точную
 * семантическую цель. Резервный поиск по Class.method или уникальному последнему
 * сегменту может связать `kotlin.IntArray.iterator` с посторонним методом проекта
 * и потому запрещён. */
...
/* Разные QN не считаются неоднозначностью только тогда, когда точный поиск
 * связывает оба с одним материализованным узлом. Иначе ссылка не разрешается. */
static inline bool cbm_pipeline_reference_targets_equal(const char *left_qn, const char *right_qn,
    ...
    /* Резервный поиск по хвосту QN вызывающей функции помогает JVM-строкам найти
     * цель, но не доказывает, что два разных QN обозначают один символ. Здесь
     * допустим только точный поиск обоих написаний в один и тот же узел. */
    (void)allow_tail_match;
...
static inline const CBMResolvedCall *cbm_pipeline_find_lsp_reference_view_in_graph(
    ...
    for (int i = 0; i < row_count; i++) {
        ...
        bool name_matches = cbm_lsp_callable_leaf_matches(rc->callee_qn, ref_leaf);
...
/* Сопоставляет локальный callee_qn от LSP с узлом графового буфера. Сначала
 * выполняется точный поиск; проектный префикс не восстанавливается. Для JVM
 * разрешён ограниченный поиск по Class.method, а при явном разрешении — по
 * единственному вызываемому short name. Счётчики ниже делают стоимость этого
 * потенциально неограниченного перебора наблюдаемой. */
