...
static int pipeline_refresh_git_context(cbm_pipeline_t *p) {
    ...
    char *fresh_branch_qn = cbm_git_context_branch_qn(&fresh);
    if (!fresh_branch_qn) {
        cbm_git_context_free(&fresh);
        return CBM_NOT_FOUND;
    }
...
static int try_incremental_or_delete_db(cbm_pipeline_t *p, cbm_file_info_t *files, int file_count,
    ...
    bool qn_format_current = valid && cbm_store_qn_format_is_current(check_store);
    if (check_store) {
    ...
    if (!qn_format_current) {
        ...
        rc = CBM_PIPELINE_FORCE_FULL_REINDEX;
    } else {
