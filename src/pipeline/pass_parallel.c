...
/* В каноническом C++ QN последний сегмент содержит сигнатуру (`render()`), а
 * промежуточная запись — исходное короткое имя (`render`). Точный узел графа
 * уже знает имя без сигнатуры; для нематериализованных целей JVM и внешних
 * библиотек сохраняется прежний резервный поиск по последнему сегменту QN. */
static const char *lsp_idx_candidate_leaf(const cbm_gbuf_t *gbuf, const char *project_name,
                                          const char *callee_qn) {
    const cbm_gbuf_node_t *target =
        cbm_pipeline_lsp_target_node_policy(gbuf, project_name, callee_qn, false, false);
    return target && target->name && target->name[0] ? target->name
                                                     : cbm_lsp_bare_segment(callee_qn);
}
        ...
        if (lsp_exact_idx || lsp_legacy_idx) {
            for (int i = 0; i < result->resolved_calls.count; i++) {
                ...
                const char *callee_leaf =
                    lsp_idx_candidate_leaf(rc->main_gbuf, rc->project_name, rc_e->callee_qn);
