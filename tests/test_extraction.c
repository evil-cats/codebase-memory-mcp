...
/* Имя привязки Nix является путём, а раньше индексатор брал только первый
 * сегмент. Правило совпадает с пространствами имён C++: у `ns::serialize`
 * короткое имя равно `serialize`, а локальный QN — `file.ns.serialize`.
 * Поэтому короткое имя привязки равно последнему сегменту, а QN включает
 * окружающую область. */
TEST(nix_attrset_scope_disambiguates_leaf_names) {
    ...
    ASSERT(count_defs_with_label(r, "Function") == 2);
    ASSERT(has_def_qn(r, "collide.setA.dup"));
    ASSERT(has_def_qn(r, "collide.setB.dup"));
    cbm_free_result(r);
    PASS();
}
...
TEST(nix_dotted_attrpath_qualifies_like_nested) {
    ...
    ASSERT(has_def_qn(r, "d.wrap.deep.fn"));
    ...
    ASSERT(has_def_qn(n, "d.wrap.deep.fn"));
    cbm_free_result(n);
    cbm_free_result(r);
    PASS();
}
...
TEST(nix_quoted_attr_name_strips_quotes) {
    ...
    ASSERT(has_def_qn(r, "q.kebab-case"));
...
TEST(nix_module_level_bindings_mint_variables) {
    ...
    ASSERT(has_def_qn(r, "mod.services.nginx.enable"));
    cbm_free_result(r);
    PASS();
}
