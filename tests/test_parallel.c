...
static const cbm_gbuf_node_t *find_unique_callable_node_by_tail(const cbm_gbuf_t *gbuf,
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (!node_tail || strncmp(node_tail, tail, tail_len) != 0 ||
            (node_tail[tail_len] != '\0' && node_tail[tail_len] != '(' &&
             node_tail[tail_len] != '<')) {
            continue;
        }
        if (match) {
...
/* QN с проектным префиксом относится к старому формату и не должен скрыто
 * становиться псевдонимом локальной цели, даже при совпадении места вызова. */
TEST(lsp_resolve_project_prefixed_row_does_not_alias_local_target) {
    const char *project = "proj";
    cbm_gbuf_t *gbuf = cbm_gbuf_new(project, "/tmp");
    ASSERT_NOT_NULL(gbuf);
    int64_t target_id = cbm_gbuf_upsert_node(gbuf, "Function", "handler", "mod.Target.handler",
                                             "target.py", 1, 1, "{}");
    ASSERT_GT(target_id, 0);

    ASSERT_NOT_NULL(cbm_pipeline_lsp_target_node(gbuf, project, "mod.Target.handler", false));
    ASSERT_NULL(cbm_pipeline_lsp_target_node(gbuf, project, "proj.mod.Target.handler", false));

    CBMCall call = make_call("mod.Caller.run", "handler");
    call.site_start_byte = 110;
    call.site_end_byte = 119;
    CBMResolvedCall local = make_rc("mod.Caller.run", "mod.Target.handler", 0.75f);
    local.site_start_byte = call.site_start_byte;
    local.site_end_byte = call.site_end_byte;
    CBMResolvedCall prefixed = make_rc("mod.Caller.run", "proj.mod.Target.handler", 0.90f);
    prefixed.site_start_byte = call.site_start_byte;
    prefixed.site_end_byte = call.site_end_byte;
    CBMResolvedCall items[] = {local, prefixed};
    CBMResolvedCallArray arr = {items, 2, 2};
...
/* Строка LSP с проектным префиксом не должна скрыто становиться псевдонимом
 * локальной цели при разрешении ссылки. Разные QN сохраняют неоднозначность. */
TEST(lsp_reference_project_prefixed_row_does_not_alias_local_target) {
    const char *project = "proj";
    cbm_gbuf_t *gbuf = cbm_gbuf_new(project, "/tmp");
    ASSERT_NOT_NULL(gbuf);
    int64_t target_id = cbm_gbuf_upsert_node(gbuf, "Function", "handler", "mod.Target.handler",
                                             "target.go", 1, 1, "{}");
    ASSERT_GT(target_id, 0);
    const cbm_gbuf_node_t *local_target =
    ...
    ASSERT_NULL(prefixed_target);

    CBMUsage usage = make_reference_usage("mod.Caller.run", "handler", 1050, 1057);
    CBMResolvedCall local =
        make_reference_rc("mod.Caller.run", "mod.Target.handler", 0.75f, 1050, 1057);
    CBMResolvedCall prefixed =
        make_reference_rc("mod.Caller.run", "proj.mod.Target.handler", 0.90f, 1050, 1057);
    CBMResolvedCall items[] = {local, prefixed};
    CBMResolvedCallArray arr = {items, 2, 2};
...
TEST(lsp_target_node_supports_long_local_qn_without_prefix_retry) {
    const char *project = "long_project";
    enum { CALLEE_LEN = 1100 };
    char *callee = malloc(CALLEE_LEN + 1U);
    char *prefixed = malloc(strlen(project) + 1U + CALLEE_LEN + 1U);
    ASSERT_NOT_NULL(callee);
    ASSERT_NOT_NULL(prefixed);
    memset(callee, 'n', CALLEE_LEN);
    callee[CALLEE_LEN] = '\0';
    snprintf(prefixed, strlen(project) + 1U + CALLEE_LEN + 1U, "%s.%s", project, callee);

    cbm_gbuf_t *gbuf = cbm_gbuf_new(project, "/tmp");
    ASSERT_NOT_NULL(gbuf);
    int64_t id = cbm_gbuf_upsert_node(gbuf, "Function", "target", callee, "target.c", 1, 1, "{}");
    ASSERT_GT(id, 0);
    const cbm_gbuf_node_t *found = cbm_pipeline_lsp_target_node(gbuf, project, callee, false);
    const cbm_gbuf_node_t *legacy = cbm_pipeline_lsp_target_node(gbuf, project, prefixed, false);
    if (!found) {
        printf("  long target-QN diagnostic: project=%zu local=%zu prefixed=%zu\n", strlen(project),
               strlen(callee), strlen(prefixed));
    }
    bool same_node = found && found->id == id && !legacy;
    cbm_gbuf_free(gbuf);
    free(prefixed);
    free(callee);
...
SUITE(parallel) {
    ...
    RUN_TEST(lsp_resolve_project_prefixed_row_does_not_alias_local_target);
    ...
    RUN_TEST(lsp_reference_project_prefixed_row_does_not_alias_local_target);
    ...
    RUN_TEST(lsp_target_node_supports_long_local_qn_without_prefix_retry);
