                        ...
                        if (pkg_name) {
                            ...
                            if (pkg_qn && field_name) {
                                ...
                                emit_unresolved_call(
                                    ctx, go_qn_join(ctx->arena, pkg_name, field_name),
                    ...
                    if (field_name) {
                        ...
                        if (base && field_name) {
                            ...
                            if (is_iface) {
                                ...
                                emit_resolved_call(
                                    ctx, go_qn_join(ctx->arena, iface_qn ? iface_qn : "interface",
                                                    field_name),
                        ...
                        if (base && base->kind == CBM_TYPE_NAMED) {
                            ...
                            emit_unresolved_call(
                                ctx, go_qn_join(ctx->arena, operand_text ? operand_text : "?",
                                                field_name),
...
static void process_function(GoLSPContext* ctx, TSNode func_node) {
    ...
    /* LSP `caller_qn` использует тот же базовый receiver, что извлечение
     * определений и унифицированный обход. Общий алгоритм также корректно
     * снимает generic-параметры `Box[T]`. */
    TSNode receiver = ts_node_child_by_field_name(func_node, "receiver", 8);
    char* recv_type_name = ts_node_is_null(receiver)
                               ? NULL
                               : cbm_go_receiver_type_name(ctx->arena, receiver, ctx->source);
