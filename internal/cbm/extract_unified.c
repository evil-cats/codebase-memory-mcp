...
static const char *objectscript_get_method_qn(CBMExtractCtx *ctx, TSNode node,
    ...
    if (strcmp(nk, "query") == 0) {
        ...
        return enclosing_class_qn ? cbm_arena_sprintf(ctx->arena, "%s.%s", enclosing_class_qn, name)
                                  : cbm_fqn_compute(ctx->arena, ctx->rel_path, name);
    }
...
static const char *compute_func_qn(CBMExtractCtx *ctx, TSNode node, const CBMLangSpec *spec,
    ...
    if (!base_qn && state->enclosing_class_qn) {
        base_qn = cbm_arena_sprintf(ctx->arena, "%s.%s", state->enclosing_class_qn, qn_name);
    }
    if (!base_qn) {
         ...
         * точному равенству). */
        base_qn = cbm_fqn_compute_source_lang(ctx->arena, ctx->rel_path, qn_name, ctx->language);
    }
    if (!is_cpp) {
        return base_qn;
    }
